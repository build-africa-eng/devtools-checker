const util = require('util'); const logger = require('../logger');
...
@returns {Promise<*>} */ async function processConsoleArg(arg, debug = false) { try { const remoteObj = arg.remoteObject(); const type = remoteObj.type; const subtype = remoteObj.subtype;

if (type === 'undefined') return { value: undefined, type }; if (type === 'function') { const code = await arg.evaluate(fn => fn.toString()).catch(() => '[Code unavailable]'); return { value: '[Function]', type, code }; } if (type === 'symbol') return { value: '[Symbol]', type }; if (type === 'bigint') return { value: remoteObj.value + 'n', type };
...
@param {boolean} debug */ function formatAndLogMessage(msg, debug = false) { const location = ${msg.url || ''}:${msg.lineNumber || -1}:${msg.columnNumber || -1}; const timestamp = new Date(msg.timestamp || Date.now()).toISOString(); const context = msg.frameUrl && msg.frameUrl !== (msg.url || '') ?  (frame: ${msg.frameUrl}) : '';


const logEntry = { level: msg.type === 'error' ? 'error' : msg.type === 'warn' ? 'warn' : msg.type === 'info' ? 'info' : 'debug', message: msg.text || 'No message', url: msg.url || '', lineNumber: msg.lineNumber || -1, columnNumber: msg.columnNumber || -1, frameId: msg.frameId || 'main', frameUrl: msg.frameUrl || '', timestamp: msg.timestamp || Date.now(), isoTimestamp: timestamp, context, args: Array.isArray(msg.args) ? msg.args : [], };

const errorArg = logEntry.args.find(arg => arg?.error); if (errorArg) { logEntry.errorDetails = { message: errorArg.message || msg.text, name: errorArg.name || 'UnknownError', stackTrace: errorArg.stack || No stack available for message: ${errorArg.message}, isIndexedDB: errorArg.isIndexedDB || false, isAbortError: errorArg.isAbortError || false, raw: errorArg.raw || '', code: errorArg.code || 'GENERIC_ERROR', args: logEntry.args.map(arg => typeof arg === 'object' ? util.inspect(arg, { depth: null }) : String(arg)), }; logEntry.message = Enhanced ${msg.type.toUpperCase()}: ${logEntry.message}; }

const jsonLog = JSON.stringify(logEntry, null, 2); loggerlogEntry.level; if (debug) console.log(jsonLog); }

module.exports = { processConsoleArg, formatAndLogMessage, };
