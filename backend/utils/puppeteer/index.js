const isValidUrl = require('../isValidUrl'); const logger = require('../logger'); const { launchBrowserWithRetries } = require('./launchBrowser'); const { setupLogging } = require('./setupLogging'); const { setupNetworkCapture } = require('./setupNetworkCapture'); const { runLighthouse } = require('./runLighthouse'); const { runAccessibility } = require('./runAccessibility'); const { setupTracing } = require('./setupTracing'); const { captureTouchAndGestureEvents, captureHtml, captureScreenshot, captureMobileMetrics, getDomMetrics, inspectElement, executeScript, } = require('./helpers'); const WebSocket = require('ws'); const fs = require('fs').promises;

async function analyzeUrl(url, options = {}) { let browser, page, wsServer; const result = {};

try { if (!isValidUrl(url)) throw new Error('Invalid URL');
