 ...
 * @returns {Promise<object>} Summary of the trace.
 */
async function parseTrace(tracePath) {
  const rawData = await fs.promises.readFile(tracePath, 'utf-8');
  ...
  for (const e of events) {
    ...
    if (e.name) {
      eventTypes.add(e.name);
      if (e.name === 'Screenshot') screenshotCount++;
    }
  }
 ...
 * @returns {() => Promise<{tracePath: string, summary: object}>}
 */
async function setupTracing(page, outputDir, debug = false) {
  const tracePath = path.join(outputDir, 'trace.json');

  try {
    if (!page) throw new Error('Invalid Puppeteer page instance');
    ...
    logger.error(`Tracing start failed: ${err.message}`);
    throw err;
  }
  ...
  return async () => {
    try {
      await Promise.race([
        page.tracing.stop(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Tracing stop timed out')), 60000)),
      ]);
